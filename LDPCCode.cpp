...
LDPCCode::LDPCCode(long N1, long K1, long W1, long I1)
: N(N1), K(K1), IMAX(I1)
// N1 = code length; K1 = message length
// W1 = column weight (must be odd)
// I1 = max number of iteration for decoding
...
long LDPCCode::decode_(Vec<GF2>& x, const double y[])
// output: x = decoding of y by sum product algorithm
// input: y = received code with gaussian noise
//          = log(prob(x==0)/prob(x==1))
...
long LDPCCode::decode(Vec<GF2>& x, const Vec<double>& y)
